...
assert.equal(totalSavings.count,4);assert.equal(totalSavings.water,8100);assert.equal(totalSavings.cost,728);
...
vm.runInContext("activeDate='*';featureFilters.both=true;featureFilters.pano=true;featureFilters.wash=true;render()",ctx);
assert.equal(vm.runInContext("sessions(filtered()).map(s=>s.primary.unit+' '+s.primary.date).sort().join(',')",ctx),'450015+450057 2026-08-19,450047+450119 2026-08-19,701020 2026-08-07,701020 2026-08-11,701030 2026-08-19,701046 2026-08-04,701046 2026-08-06');
assert.equal(vm.runInContext('savings(filtered()).count',ctx),3);
...
console.log('Side isolation, supplied covers, final-date washes, combined filters and 8100 L / £728 savings passed');
