...
assert.equal(catalog.events.filter(e=>/(0919|0920)$/.test(e.sourceFolder)).length,23);
...
assert.equal(totalSavings.count,3);assert.equal(totalSavings.water,6300);assert.equal(totalSavings.cost,546);
vm.runInContext("activeDate='*';featureFilters.both=true;featureFilters.pano=true;featureFilters.wash=true;render()",ctx);
assert.equal(vm.runInContext("sessions(filtered()).map(s=>s.primary.unit+' '+s.primary.date).sort().join(',')",ctx),'701020 2026-08-07,701020 2026-08-11,701030 2026-08-19,701046 2026-08-04,701046 2026-08-06');
assert.equal(vm.runInContext('savings(filtered()).count',ctx),2);
...
assert.equal((history.match(/wash-opportunity-frame/g)||[]).length,20);
...
console.log('Side isolation, supplied covers, final-date washes, combined filters and 6300 L / £546 savings passed');
