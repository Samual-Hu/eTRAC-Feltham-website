...
async function enableDevelopmentEditing(){
  ...
  capture.carriages.forEach((carriage)=>{const row=table.querySelector(`[data-row="${carriage.serial}"]`);const cell=row?.children[2];if(!cell)return;cell.innerHTML=`<select class="dev-grade" aria-label="Edit cleanliness for ${carriage.serial}"><option value="" disabled>Not assessed</option><option value="compliant">Compliant</option><option value="marginal">Marginal</option><option value="non-compliant">Non-compliant</option></select>`;const select=cell.querySelector('select');select.value=carriage.assessed?gradeValue(carriage.cleanliness):'';select.addEventListener('change',async()=>{select.disabled=true;await window.EyyaDev.save({action:'grade',side:capture.side,serial:carriage.serial,date:capture.date,grade:select.value,seedAnnotations:seedAnnotations(carriage)});location.reload();});});
  capture.carriages.forEach((carriage)=>{
    const row=table.querySelector(`[data-row="${carriage.serial}"]`),cell=row?.children[1];if(!cell)return;
    const single=document.createElement('button');single.type='button';single.className='dev-single-annotate';single.textContent='标注本图';single.setAttribute('aria-label',`单独标注 ${carriage.serial} 当前日期`);
    single.addEventListener('click',async()=>{single.disabled=true;try{await window.EyyaDev.launchAnnotation({image:carriage.image});badge.textContent='APP STARTED';setTimeout(()=>badge.textContent='LOCAL EDIT',2500);}catch(error){alert(error.message);}finally{single.disabled=false;}});
    cell.append(single);
    const choices=catalog.events.filter((item)=>item.id!==capture.id&&item.mode==='panorama'&&(!capture.side||item.side===capture.side)&&item.carriages.some((candidate)=>candidate.serial===carriage.serial));
    if(!choices.length)return;
    const select=document.createElement('select');select.className='dev-compare-date';select.setAttribute('aria-label',`Open local annotation app for ${carriage.serial}`);select.innerHTML='<option value="">对比标注…</option>'+choices.sort((a,b)=>a.date.localeCompare(b.date)).map((item)=>`<option value="${item.id}">${readableDate(item.date)}</option>`).join('');
  ...
  editor.querySelector('[data-dev-save]').addEventListener('click',async()=>{if(!active)return;await window.EyyaDev.save({action:'annotation',side:capture.side,serial:active.carriage.serial,date:capture.date,annotationId:active.defect.id,type:editor.querySelector('[data-dev-type]').value,bbox:active.defect.box,seedAnnotations:seedAnnotations(active.carriage)});location.reload();});
  editor.querySelector('[data-dev-delete]').addEventListener('click',async()=>{if(!active||!confirm('Delete this reviewed region?'))return;await window.EyyaDev.save({action:'delete',side:capture.side,serial:active.carriage.serial,date:capture.date,annotationId:active.defect.id,seedAnnotations:seedAnnotations(active.carriage)});location.reload();});
}
